...
async function validateESLintRules() {
  ...
  console.log("\n📊 ESLint Results:");
  console.log(
    `  Violations caught: ${violationsCaught}/${testCases.violations.length}`
  );
  console.log(`  Valid cases passed: ${validPassed}/${testCases.valid.length}`);

  return (
    violationsCaught === testCases.violations.length &&
    validPassed === testCases.valid.length
  );
}
...
async function validateTailwindSafelist() {
  ...
  try {
    ...
    if (!fs.existsSync(tailwindConfigPath)) {
      console.log(
        "⚠️  No tailwind.config.js found - run /2-setup-foundation first"
      );
      return false;
    }
...
async function main() {
  ...
  console.log(
    `  DESIGN_SYSTEM.md: ${designSystemValid ? "✅ PASS" : "❌ FAIL"}`
  );
  ...
  if (eslintValid && safelistValid && designSystemValid) {
    ...
    if (!safelistValid)
      console.log("  - Run /2-setup-foundation after setup-design");
    if (!eslintValid)
      console.log("  - Check ESLint configuration in eslint.config.js");
    process.exit(1);
  }
}
